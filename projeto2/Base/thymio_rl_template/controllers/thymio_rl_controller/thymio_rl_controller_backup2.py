...
class OpenAIGymEnvironment(Supervisor, gym.Env):
    ...
    def step(self, action):
        ...
        proximity_penalty = -np.sum(np.exp(-np.array(prox_values))) +10  # closer = lower
        ground_penalty = np.sum(np.array(ground_values) < 0.1)  # near edge or drop
        linear_velocity_reward = np.mean(action)*1.5  # prefer moving forward
...
def main():
    ...
    checkpoint_callback = CheckpointCallback(save_freq=1000, save_path='modelos/', name_prefix='sexto_modelo_nao_finalizado')

    # Code to load a model and run it
    # For the RecurrentPPO case, consult its documentation
    model = PPO("MlpPolicy", env, verbose=1 )
    model.learn(total_timesteps=100000, callback=checkpoint_callback)
    model.save("sexto_modelo_final")
    
    
        # dar load ao modelo
    """model = PPO.load("quinto_modelo_final")
    obs, _ = env.reset()
    lstm_states = None
