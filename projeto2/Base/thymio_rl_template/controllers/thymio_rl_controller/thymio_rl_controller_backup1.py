...
class OpenAIGymEnvironment(Supervisor, gym.Env):
    ...
    def step(self, action):
        ...
        reward = linear_velocity_reward - teste - ground_penalty
    ...
    def testes(self,steps):
        ...
        for i in range(steps):
            self.step([5.0, 5.0])  # Forward movement
...
def main():
    ...
    """checkpoint_callback = CheckpointCallback(save_freq=1000, save_path='modelos/', name_prefix='modelo_nao_finalizado')

    # Code to load a model and run it
    # For the RecurrentPPO case, consult its documentation
    model = PPO("MlpPolicy", env, verbose=1 )
    model.learn(total_timesteps=100000, callback=checkpoint_callback)
    model.save("primeiro_modelo_final")
    
    
    print("Running trained model...")
    obs, _ = env.reset()
    for _ in range(250):  # number of steps to run the model
        action, _ = model.predict(obs)
        obs, reward, terminated, truncated, _ = env.step(action)

        ground_values = [s.getValue() / 4096.0 for s in env.sensores_baixo]
        print(f"Ground: {ground_values[0]:.3f}, {ground_values[1]:.3f}")
